...
def replace_id(df_in, field_to_sub, new_field_name, cw_path, cnt):
    ...
    #crosswalk_df = df_in[[field_to_sub,new_field_name]].drop_duplicates().reset_index(drop=True)
    ##crosswalk_df = pd.DataFrame.from_dict(crosswalk_dict)
    
    return df_subbed, crosswalk_dict, new_field_name
###################################################################
...
def main():
    ...
    field_list = [{'field_to_sub' : 'MRN'
                   ...
                   , 'number_to_start' : 10000   ##This should be replaced by a method of substitute
                   }
    ...
    for f in field_list:
        ...
        deident_df, crosswalk_dict, new_field_name = replace_id(deident_df, f['field_to_sub'], f['new_field_name'], f['crosswalk_path'], f['number_to_start'])
        
        ## Add crosswalk for field to crosswalks list
        #crosswalks.append({f['field_to_sub']:crosswalk_df})
        crosswalks.append({f['field_to_sub']:crosswalk_dict})
    ...
    for c in crosswalks:
        for k,v in c.items():
            file_out = '%s_%s_crosswalk_%s.csv' % (in_file_name, k, today)  ##build output file name
#            v.to_csv( file_out, index=False, encoding='utf-8-sig')

            with open(file_out, 'w', newline="") as csv_file:  
                writer = csv.writer(csv_file)
                writer.writerow([k,new_field_name])
                for key, value in c[k].items():
